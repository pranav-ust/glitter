...
Each passage consists of four sentences: two as preceding context, one containing the seed word (matching sentence), and one as trailing context. The seed word appears in plural form (e.g., dispatchers). Annotators were instructed to:

* Annotate all human entities in the matching sentence, including the seed noun
* Indicate whether the gender of the seed word is \textbf{ambiguous}
* If unambiguous, annotate spans that disambiguate the gender in any of the sentences (preceding, matching, or trailing)
* Specify the seed's gender as either \textbf{feminine}, \textbf{masculine}, \textbf{non-binary}, \textbf{all genders}, or \textbf{feminine and masculine}

For example, in the sentence "In theatrical performances the use of decorative electricity was used in costumes of \textbf{female performers}," the gender of the seed word `performers' is disambiguated by the adjective `female'.
...
* For non-binary/all genders/feminine and masculine/ambiguous sources, provide \textbf{three gender-fair alternatives}:
